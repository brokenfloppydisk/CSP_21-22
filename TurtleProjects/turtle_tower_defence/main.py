...
cell_scale = 0.5
field_offset = 100
enemy_spawn_interval = (5, 8) # Min and max possible seconds between each enemy spawn
...
async def main_loop():
  ...
  while (s.game_running):
    ...
    for acting_tower in s.all_towers:
      acting_tower.act(writer)
...
def standard_tower():
  ...
  print("Standard Tower (1) selected.")
  current_tower = "Standard Tower"

def sniper_tower():
  global current_tower
  print("Sniper Tower (2) selected.")
  current_tower = "Sniper Tower"

def start(x, y):
  turtle.onkeypress(None, key="1")
  turtle.onkeypress(None, key="2")
...
def spawn_tower(mouse_x, mouse_y):
  if (mouse_inside_field(mouse_x, mouse_y)):
    ...
    if s.money >= Cost:
      s.money -= Cost
      text.update_text(writer, s.health, s.money, score)
      new_tower = tower.tower(pos=(mouse_x, mouse_y), damage=towers["Damage"][tower_name], attack_radius=towers["Attack_Radius"][tower_name], proj_speed=towers["Projectile_Speed"][tower_name], attack_cooldown=towers["Cooldown"][tower_name], color=towers["Color"][tower_name])
...
def draw_field():
  ...
  print("Press a number on your keyboard to select a tower, and click to place it on the field.\nClick the green start button when you're done.")
  turtle.onkeypress(standard_tower, "1")
  turtle.onkeypress(sniper_tower, "2")
  turtle.listen()
