...
def update_leaderboard(file_name, leader_names, leader_scores, player_name, player_score):
    ...
    # insert the new player and score at the appropriate position
...
def draw_leaderboard(leader_names, leader_scores, high_scorer, turtle_object, player_score, prev_turtles):

    # clear the screen and move turtle object to (-200, 100) to start drawing the leaderboard
    font_setup = ("Helvetica", 20, "normal")
    turtle_object.clear()
    for t in prev_turtles:
        t.clear()
    turtle_object.penup()
    ...
    if (len(leader_scores) >= 3 and player_score == int(leader_scores[2])):
        turtle_object.write("You earned a bronze medal!", font=font_setup)
    elif (len(leader_scores) >= 2 and player_score == int(leader_scores[1])):
        turtle_object.write("You earned a silver medal!", font=font_setup)
    elif (len(leader_scores) >= 1 and player_score == int(leader_scores[0])):
